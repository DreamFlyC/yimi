		...
		$("#delete").click(function() {
			...
			if (ids != "") {
				if (window.confirm('你确定要删除吗？')) {
					...
					$.ajax({
						url : "manage/crmstock/delete?ids="
...
<body>
	...
	<div class="rightContainer">
		...
		<form action="manage/crmstock/crmstock_list" method="post" onsubmit="return isValid(this)"
			...
			<div style="margin:20px 0px;">
				...
				<div class="col-md-2">
					...
					<input type="submit" id="commit" value="查&nbsp;询" class="form-control btn-primary">
				</div>
		...
		<button class="btn btn-primary"
			onclick="LW.location.forward('<%=basePath%>manage/crmstock/post.html')">
			<span class="glyphicon glyphicon-plus"></span> 新增
		</button>
		<button class="btn btn-primary"
			onclick="LW.location.forward('<%=basePath%>manage/crmstock/crmstock_fruit')">
		...
		<button type="button" class="btn btn-primary" id="statistics"
				onclick="LW.location.forward('<%=basePath%>manage/crmstock/crmstock_statistics')">
			...
			<tbody>
				<c:forEach items="${pager.datas}" var="item">
					...
					<tr id="id${item.id}">
						...
						<td>
                            <a href="<%=basePath%>manage/itemstock/${item.id}.html">编辑</a>
                        </td>
					</tr>
				</c:forEach>
			</tbody>
		...
		<nav>
			...
			<div class="btn-group pull-right" role="group">
				<a href="<%=basePath%>manage/crmstock/crmstock_list.html"
					class="btn btn-default">首页</a> <a
					href="<%=basePath%>manage/crmstock/crmstock_list.html?pageNo=${pager.prePage}"
					class="btn btn-default">上一页</a> <a
					href="<%=basePath%>manage/crmstock/crmstock_list.html?pageNo=${pager.nextPage}"
					class="btn btn-default">下一页</a> <a
					href="<%=basePath%>manage/crmstock/crmstock_list.html?pageNo=${pager.totalPage}"
    ...
    <script type="text/javascript">
        ...
        $("#titleitem").click(function() {
            ...
            if (!key) {
                $.ajax({
                    url:"manage/crmsupplier/getListBySupplier",
                    type: "post",
                    error: function() {
        ...
        function AutoComplete(auto, search, mytitle_list) {
            ...
            if ($("#" + search).val() != old_value || old_value == "") {
                ...
                for (i in titlelist) {
                    ...
                    newDivNode.attr("style", "font:14px/25px arial;height:30px;padding:0 8px;cursor: pointer;text-align:center;");
